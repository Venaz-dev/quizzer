...
class Quiz extends React.Component{
        ...
        this.myInterval = setInterval(() => {
            ...
            if (seconds === 0) {
                if (minutes === 0) {
                    ...
                    this.setState(
                        ({minutes}) => ({minutes: minutes -1, seconds: 59})
                    )
                }
            }
        }, 1000)
    ...
    render(){
        ...
        return(
            <div>
                <Header />
                <div id='quiz-timer'className='timer'>
                    <h1> Time <p>{minutes}:{seconds < 10 ? `0${seconds}` : seconds}</p></h1>
                </div>
                {this.state.message === ''? null : <h2>{this.state.message}</h2>}
                <div >
                    {submitQuiz ? <EndQuiz timeup={this.state.timeOver} score={this.state.score}/> : ListQuestion}
                </div> 
            </div>
        )
    }
